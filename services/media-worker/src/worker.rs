...
const SERVICE_NAME: &str = "media-worker";
...
impl Worker {
    ...
    {
        info!(service = self.service_name(), "media worker started");
        ...
        loop {
            tokio::select! {
                shutdown_result = &mut shutdown => {
                    shutdown_result?;
                    info!(service = self.service_name(), "media worker shutting down cleanly");
                    break;
                }
                _ = heartbeat.tick() => {
                    trace!(service = self.service_name(), "media worker waiting for future jobs");
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn default_service_name_is_media_worker() {
